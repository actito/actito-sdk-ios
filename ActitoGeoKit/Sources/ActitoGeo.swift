...
private let FAKE_BEACON_IDENTIFIER = "ActitoFakeBeacon"
...
public final class ActitoGeo: NSObject, CLLocationManagerDelegate {
    ...
    private var monitoredRegionsLimit: Int {
        guard let options = Actito.shared.options else {
    ...
    private func checkPrerequisites() throws {
        ...
        guard application.services[ActitoApplication.ServiceKey.locationServices.rawValue] == true else {
            logger.warning("Actito location functionality is not enabled.")
            throw ActitoError.serviceUnavailable(service: ActitoApplication.ServiceKey.locationServices.rawValue)
        }
    }
    ...
    private func handleLocationServicesAuthorized(monitorSignificantLocationChanges: Bool) {
        ...
        if monitorSignificantLocationChanges, Actito.shared.options?.visitsApiEnabled == true {
        ...
        for region in LocalStorage.monitoredRegions.filter({ $0.isPolygon }) {
            ...
            if !hasEntered && isInside {
                ...
                DispatchQueue.main.async {
                    self.delegate?.actito(self, didEnter: region)
                }
                ...
                DispatchQueue.main.async {
                    self.delegate?.actito(self, didExit: region)
                }
            }
        }
    }
    ...
    private func saveLocation(_ location: CLLocation) async {
        ...
        let payload = ActitoInternals.PushAPI.Payloads.UpdateDeviceLocation(
    ...
    private func loadNearestRegions(_ location: CLLocation) async {
        do {
            let response = try await ActitoRequest.Builder()
                ...
                .responseDecodable(ActitoInternals.PushAPI.Responses.FetchRegions.self)
    ...
    private func monitorRegions(_ regions: [ActitoRegion]) {
    ...
    internal func stopMonitoringLocationUpdates() {
        ...
        if Actito.shared.options?.visitsApiEnabled == true {
            logger.debug("Stopped monitoring visits.")
            locationManager.stopMonitoringVisits()
        }

        if Actito.shared.options?.headingApiEnabled == true {
    ...
    private func triggerRegionEnter(_ region: ActitoRegion) {
        guard let device = Actito.shared.device().currentDevice else {
        ...
        let payload = ActitoInternals.PushAPI.Payloads.RegionTrigger(
            deviceID: device.id,
            region: region.id
        )
    ...
    private func triggerRegionExit(_ region: ActitoRegion) {
        guard let device = Actito.shared.device().currentDevice else {
    ...
    private func triggerBeaconEnter(_ beacon: ActitoBeacon) {
        guard let device = Actito.shared.device().currentDevice else {
        ...
        let payload = ActitoInternals.PushAPI.Payloads.BeaconTrigger(
            deviceID: device.id,
            beacon: beacon.id
        )
    ...
    private func triggerBeaconExit(_ beacon: ActitoBeacon) {
        guard let device = Actito.shared.device().currentDevice else {
    ...
    private func startRegionSession(_ region: ActitoRegion) {
    ...
    private func updateRegionSession(_ location: CLLocation) {
        ...
        LocalStorage.regionSessions = LocalStorage.regionSessions.map { session in
            ...
            return ActitoRegionSession(
                ...
                locations: session.locations.appending(ActitoLocation(cl: location))
            )
        }
    }

    private func stopRegionSession(_ region: ActitoRegion) {
        ...
        Task {
            ...
            do {
                try await Actito.shared.events().logRegionSession(session)
    ...
    private func startBeaconSession(_ beacon: ActitoBeacon) {
    ...
    private func stopBeaconSession(_ beacon: ActitoBeacon) {
        guard let region = LocalStorage.monitoredRegions.first(where: { $0.id == beacon.id }) else {
        ...
        Task {
            do {
                try await Actito.shared.events().logBeaconSession(session)
    ...
    private func startMonitoringBeacons(in region: ActitoRegion) {
        ...
        guard let uuidStr = Actito.shared.application?.regionConfig?.proximityUUID,
        ...
        Task {
            do {
                let response = try await ActitoRequest.Builder()
                    ...
                    .responseDecodable(ActitoInternals.PushAPI.Responses.FetchBeacons.self)
    ...
    private func startMonitoringBeacon(_ beacon: ActitoBeacon, with uuid: UUID) {
    ...
    private func stopMonitoringBeacons(in region: ActitoRegion) {
    ...
    private func handleRangingBeacons(_ clBeacons: [CLBeacon], in clr: CLBeaconRegion) {
        ...
        clBeacons
            ...
            .forEach { clb in
                if var beacon = LocalStorage.monitoredBeacons.first(where: { $0.major == clb.major.intValue && $0.minor == clb.minor.intValue }) {
                    ...
                    beacon.proximity = ActitoBeacon.Proximity(clb.proximity) ?? .unknown
        ...
        DispatchQueue.main.async {
            // Notify the delegate.
            self.delegate?.actito(self, didRange: beacons, in: region)
        }
    }
    ...
    private func handleRangingBeaconsError(_ error: Error, for clr: CLBeaconRegion) {
        guard clr.identifier == FAKE_BEACON_IDENTIFIER else {
            ...
            DispatchQueue.main.async {
                self.delegate?.actito(self, didFailRangingFor: region, with: error)
            }
    ...
    private func updateBluetoothState(enabled: Bool) {
        ...
        if hasBluetoothEnabled != enabled {
            let payload = ActitoInternals.PushAPI.Payloads.BluetoothStateUpdate(
    ...
    public func locationManager(_: CLLocationManager, didFailWithError error: Error) {
        ...
        DispatchQueue.main.async {
            self.delegate?.actito(self, didFailWith: error)
        }
    }
    ...
    public func locationManager(_: CLLocationManager, didStartMonitoringFor clr: CLRegion) {
        ...
        if clr is CLBeaconRegion {
            ...
            DispatchQueue.main.async {
                self.delegate?.actito(self, didStartMonitoringFor: beacon)
            }
        } else {
            guard let region = LocalStorage.monitoredRegions.first(where: { $0.id == clr.identifier }) else {
            ...
            DispatchQueue.main.async {
                self.delegate?.actito(self, didStartMonitoringFor: region)
            }
        }
    }
    ...
    public func locationManager(_: CLLocationManager, monitoringDidFailFor clr: CLRegion?, withError error: Error) {
        ...
        if clr is CLBeaconRegion {
            ...
            DispatchQueue.main.async {
                self.delegate?.actito(self, monitoringDidFailFor: beacon, with: error)
            }
        } else {
            guard let region = LocalStorage.monitoredRegions.first(where: { $0.id == clr.identifier }) else {
            ...
            DispatchQueue.main.async {
                self.delegate?.actito(self, monitoringDidFailFor: region, with: error)
            }
        }
    }
    ...
    public func locationManager(_ manager: CLLocationManager, didEnterRegion clr: CLRegion) {
        if clr is CLBeaconRegion {
            ...
            DispatchQueue.main.async {
                self.delegate?.actito(self, didEnter: beacon)
            }
        } else {
            guard let region = LocalStorage.monitoredRegions.first(where: { $0.id == clr.identifier }) else {
    ...
    public func locationManager(_ manager: CLLocationManager, didExitRegion clr: CLRegion) {
        if clr is CLBeaconRegion {
            ...
            DispatchQueue.main.async {
                self.delegate?.actito(self, didExit: beacon)
            }
        } else {
            // Trigger a location update in order to update the loaded fences as a side effect.
            manager.requestLocation()
    ...
    public func locationManager(_: CLLocationManager, didDetermineState state: CLRegionState, for clr: CLRegion) {
        ...
        if clr is CLBeaconRegion {
            ...
            DispatchQueue.main.async {
                self.delegate?.actito(self, didDetermineState: state, for: beacon)
            }
        } else {
            guard let region = LocalStorage.monitoredRegions.first(where: { $0.id == clr.identifier }) else {
                ...
                state == .inside || state == .outside
            {
                ...
                DispatchQueue.main.async {
                    self.delegate?.actito(self, didDetermineState: newState, for: region)
                }
            ...
            DispatchQueue.main.async {
                self.delegate?.actito(self, didDetermineState: state, for: region)
            }
        }
    }
    ...
    public func locationManager(_: CLLocationManager, didVisit visit: CLVisit) {
        ...
        Task {
            do {
                try await Actito.shared.events().logVisit(visit)
        ...
        DispatchQueue.main.async {
            self.delegate?.actito(self, didVisit: visit)
        }
    }
    ...
    public func locationManager(_: CLLocationManager, didUpdateHeading newHeading: CLHeading) {
        ...
        DispatchQueue.main.async {
            self.delegate?.actito(self, didUpdateHeading: heading)
        }
    }
