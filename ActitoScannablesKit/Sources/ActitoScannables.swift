...
public protocol ActitoScannables: AnyObject {
    // MARK: Properties
    ...
    var delegate: ActitoScannablesDelegate? { get set }
    ...
    func fetch(tag: String, _ completion: @escaping ActitoCallback<ActitoScannable>)
    ...
    func fetch(tag: String) async throws -> ActitoScannable
}

extension ActitoScannables {
    /// Starts a QR code scannable session.
    ///
    /// Initiates a QR code-based scan using the device camera, allowing the user to scan QR codes.
    ///
