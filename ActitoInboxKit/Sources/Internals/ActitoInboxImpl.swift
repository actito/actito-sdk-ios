...
internal class ActitoInboxImpl: NSObject, ActitoModule, ActitoInbox {
    ...
    internal var _items: [ActitoInboxItem] = []
    ...
    private func addToLocalInbox(_ item: ActitoInboxItem, visible: Bool) async throws {
        ...
        await database.backgroundContext.performCompat {
            self.cachedEntities.forEach { entity in
                if entity.expired, let notificationId = entity.notificationId {
                    Actito.shared.removeNotificationFromNotificationCenter(notificationId)
                }
            }
        }
    }
