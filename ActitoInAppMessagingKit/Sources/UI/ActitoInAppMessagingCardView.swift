...
public class ActitoInAppMessagingCardView: UIView, ActitoInAppMessagingView {
    public let message: ActitoInAppMessage
    public let cache: ActitoImageCache
    public weak var delegate: ActitoInAppMessagingViewDelegate?
...
//            cardView.trailingAnchor.constraint(lessThanOrEqualTo: contentView.trailingAnchor, constant: -16),
    ...
    public init(message: ActitoInAppMessage, cache: ActitoImageCache) {
    ...
    // MARK: - ActitoInAppMessagingView

    public func animate(transition: ActitoInAppMessagingViewTransition, _ completion: @escaping () -> Void) {
        switch transition {
        case .enter:
    ...
    private func canShowAction(_ action: ActitoInAppMessage.Action?) -> Bool {
